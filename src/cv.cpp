...
using namespace arma;
  ...
  void CV::IteratePartition(){
    ...
    //Use fit object to create matrix of beta values for our lambda vector using training data for this partition.
    ...
    for(int i=0;i<100;i++){
    results_(i,partition_counter_)= sum(square(betamat.row(i).t() - test_y))/test_x.n_rows;
    }
